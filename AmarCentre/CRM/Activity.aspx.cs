...
namespace AmarCentre.CRM
{
    public partial class Activity : System.Web.UI.Page
    {
        ...
        {
            DataTable dtPriorityList = masterBAL.GetActivityList(PageNumber, PageSize, Filter, OrderByColumnName, OrderBy);
            rptList.DataSource = dtPriorityList;
            rptList.DataBind();
            if (dtPriorityList.Rows.Count > 0)
            {
                lblPageInfo.Text = "Showing Results " + dtPriorityList.Rows[0]["StartNumber"].ToString() + " - " + dtPriorityList.Rows[dtPriorityList.Rows.Count - 1]["RowNum"].ToString() + " Out of " + dtPriorityList.Rows[0]["CurrentCount"].ToString() + " Records";
                hdnFilter.Value = dtPriorityList.Rows[0]["Filter"].ToString();
                hdnOrderByColumnName.Value = dtPriorityList.Rows[0]["OrderByColumnName"].ToString();
                hdnOrderBy.Value = dtPriorityList.Rows[0]["OrderBy"].ToString();
                hdnLastPage.Value = dtPriorityList.Rows[0]["LastPage"].ToString();
                lblPageNumber.Text = dtPriorityList.Rows[0]["PageNumber"].ToString();
                hdnTotal.Value = dtPriorityList.Rows[0]["CurrentCount"].ToString();
        ...
        {
            DataTable dtPriorityList = masterBAL.GetActivityListExcel();
            if (dtPriorityList.Rows.Count > 0)
            {
                StringWriter sw = systemUtilities.ExportToExcel(dtPriorityList, "ActivityListExcel");
        ...
        protected void rptListOnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataTable dtBasic = masterBAL.EditActivity(Convert.ToInt32(hdnRptId.Value));
        ...
        {
            int res = masterBAL.InsertUpdateActivity(Convert.ToInt32(hdnId.Value), txtName.Text,
        ...
        {
            int res = masterBAL.DeleteActivity(Convert.ToInt32(hdnId.Value), Convert.ToInt32(hdnUserId.Value));
            if (res == 1)
            {
                fillgridList(Convert.ToInt32(lblPageNumber.Text), Convert.ToInt32(drpPageSize.SelectedValue), hdnFilter.Value, hdnOrderByColumnName.Value, hdnOrderBy.Value);
                Clear();
        ...
        public void CheckPrivilege()
        {
            try
            {
                if (hdnUserId.Value != null)
                {
                    ...
                    int val = systemUtilities.Form_Previlage_Validation(146, Convert.ToInt32(hdnUserId.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void CheckActionPrivilege()
        {
            try
            {
                if (hdnUserId.Value != null)
                {
                    ...
                    DataTable dtSubMenuAction = systemUtilities.Action_Previlage_Validation(146, Convert.ToInt32(hdnUserId.Value));
