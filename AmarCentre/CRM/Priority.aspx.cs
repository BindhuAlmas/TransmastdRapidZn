...
namespace AmarCentre.CRM
{
    public partial class Priority : System.Web.UI.Page
    {
        ...
        {
            DataTable dtPriorityList = masterBAL.GetPriorityList(PageNumber, PageSize, Filter, OrderByColumnName, OrderBy);
        ...
        {
            DataTable dtPriorityList = masterBAL.GetPriorityListExcel();
            if (dtPriorityList.Rows.Count > 0)
            {
                StringWriter sw = systemUtilities.ExportToExcel(dtPriorityList, "PriorityListExcel");
        ...
        protected void rptListOnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataTable dtBasic = masterBAL.EditPriority(Convert.ToInt32(hdnRptId.Value));
        ...
        {
            int res = masterBAL.DeletePriority(Convert.ToInt32(hdnId.Value), Convert.ToInt32(hdnUserId.Value));
            if (res == 1)
            {
                fillgridList(Convert.ToInt32(lblPageNumber.Text), Convert.ToInt32(drpPageSize.SelectedValue), hdnFilter.Value, hdnOrderByColumnName.Value, hdnOrderBy.Value);
                Clear();
