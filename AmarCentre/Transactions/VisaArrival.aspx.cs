...
{
    public partial class VisaArrival : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_trans.Get_ListVisaArrival(page_number, page_size, filter);
        ...
        {
            DataTable dt = obj_trans.VisaArrival_Excel();
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "VisaArrival");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_trans.EditVisaArrival(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];

            hdn_id.Value = dt.Rows[0]["Id"].ToString();
            job_date.DbSelectedDate = dt.Rows[0]["Dated"].ToString();
            ReachedDate.DbSelectedDate = dt.Rows[0]["FromDate"].ToString();
            returndate.DbSelectedDate = dt.Rows[0]["ToDate"].ToString();
        ...
        {
            int res = obj_trans.InsertUpdateVisaArrival(Convert.ToInt32(hdn_id.Value), txtname.Text,
               job_date.SelectedDate, ReachedDate.SelectedDate, returndate.SelectedDate);
            if (res >0)
            {
        ...
        {
            int res = obj_trans.DeleteVisaArrival(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(75, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(75, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
