...
namespace AmarCentre.Transactions
{
    public partial class LeaveEntry : System.Web.UI.Page
    {
        ...
        private void FillDrp()
        {
            ...
            drpLeaveType.DataSource = obj_master.DrpLeaveType();
            drpLeaveType.DataTextField = "text";
            drpLeaveType.DataValueField = "value";
            drpLeaveType.DataBind();
        }

        public void grid_fill(int page_number, int page_size, string filter, string column, string order)
        {
            DataTable dt = obj_master.Get_List_LeaveEntry(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_List_LeaveEntry_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "LeaveEntry");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.Edit_LeaveEntry(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            drpEmployee.SelectedValue = dt.Rows[0]["EmployeeId"].ToString();
            drpLeaveType.SelectedValue = dt.Rows[0]["leavetypeid"].ToString();
            lbl_Code.Text = dt.Rows[0]["Code"].ToString();
            from_date.DbSelectedDate = dt.Rows[0]["fromdate"].ToString();
            to_date.DbSelectedDate = dt.Rows[0]["todate"].ToString();
        ...
        {
            int res = obj_master.Insert_Update_LeaveEntry(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(drpEmployee.SelectedValue),Convert.ToInt32(drpLeaveType.SelectedValue),from_date.SelectedDate.Value,to_date.SelectedDate.Value, txtreason.Text,
        ...
        {
            int res = obj_master.Delete_LeaveEntry(Convert.ToInt32(hdn_id.Value),Convert.ToInt32(hdn_user_id.Value));
            if (res >0)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            DataTable dt = obj_common.Get_Code(147);
            if (dt.Rows.Count > 0)
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(148, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(148, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
