...
{
    public partial class EmployeeSalaryProcess : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_trans.Get_List_EmployeeSalaryProcess(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_trans.Get_List_EmployeeSalaryProcess_Excel();
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "EmployeeSalaryProcess");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_trans.Edit_EmployeeSalaryProcess(Convert.ToInt32(hdn_rpt_id.Value));
            ...
            txtTotal.Text = dt1.Rows[0]["Amount"].ToString();
        ...
        protected void btn_save_OnClick(object sender, EventArgs e)
        {
            ...
            {
                res = obj_trans.Insert_Update_EmployeeSalaryProcess(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(drpMonth.SelectedValue),
                    Convert.ToInt32(drpYear.SelectedValue), Convert.ToDecimal(txtTotal.Text), txtRemark.Text,
                    dt_deatils, Convert.ToInt32(hdn_user_id.Value), job_date.SelectedDate);
            }
            else
            {
                lbl_msgin.Text = "Add Employee Salary to Continue !..";
        ...
        protected void btnSavePrintOnClick(object sender, EventArgs e)
        {
            DataTable dt_deatils = fill_Detail();
            ...
            if (res > 0)
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDiv();", true);
                 string url = "";
                 url = "../Reports/SalaryProcessPrint.aspx?Id=" + res;
        ...
        {
            string url = "";
            url = "../Reports/SalaryProcessPrint.aspx?id=" + Convert.ToInt32(hdn_id.Value);
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "NewWindow", "window.open('" + url + "','_blank','height=600,width=900,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=no,titlebar=no' );", true);
        }
        ...
        public DataTable fill_Detail()
        {
            ...
            dt_ser.Columns.Add("EmpSalaryProcessDId", typeof(int));
            dt_ser.Columns.Add("EmpAttendanceDId", typeof(int));
            dt_ser.Columns.Add("EmployeeId", typeof(int));
            dt_ser.Columns.Add("Salary", typeof(decimal));
            dt_ser.Columns.Add("Addition", typeof(decimal));
            dt_ser.Columns.Add("Deduction", typeof(decimal));
            dt_ser.Columns.Add("IncentiveAmount", typeof(decimal));
            dt_ser.Columns.Add("TotalSalary", typeof(decimal));

            if (rpt_Item_list.Items.Count > 0)
            {
                foreach (RepeaterItem itm in rpt_Item_list.Items)
                {
                    HiddenField hdnEmpSalaryProcessDId = (HiddenField)itm.FindControl("hdnEmpSalaryProcessDId");
                    HiddenField hdnEmpAttendanceDId = (HiddenField)itm.FindControl("hdnEmpAttendanceDId");
                    HiddenField hdnEmployeeId = (HiddenField)itm.FindControl("hdnEmployeeId");

                    TextBox txtSalary = (TextBox)itm.FindControl("txtSalary");
                    TextBox txtAddition = (TextBox)itm.FindControl("txtAddition");
                    TextBox txtDeduction = (TextBox)itm.FindControl("txtDeduction");
                    //TextBox txtIncentiveAmount = (TextBox)itm.FindControl("txtIncentiveAmount");
                    TextBox txtTotalSalary = (TextBox)itm.FindControl("txtTotalSalary");

                    dt_ser.Rows.Add(Convert.ToInt32(hdnEmpSalaryProcessDId.Value), Convert.ToInt32(hdnEmpAttendanceDId.Value), Convert.ToInt32(hdnEmployeeId.Value), 
                        Convert.ToDecimal(txtSalary.Text), txtAddition.Text == "" ? (decimal?)null : Convert.ToDecimal(txtAddition.Text),
                        txtDeduction.Text == "" ? (decimal?)null : Convert.ToDecimal(txtDeduction.Text),
                        0,//txtIncentiveAmount.Text == "" ? (decimal?)null : Convert.ToDecimal(txtIncentiveAmount.Text),
                            Convert.ToDecimal(txtTotalSalary.Text));
                }
            }
            return dt_ser;
        }

        protected void EmployeeDetails(object sender, EventArgs e)
        {
            txtTotal.Text = "";
            DataTable dt_ser = new DataTable();
            dt_ser.Columns.Add("EmpSalaryProcessDId", typeof(int));
            dt_ser.Columns.Add("EmpAttendanceDId", typeof(int));
            dt_ser.Columns.Add("EmployeeId", typeof(int));
            dt_ser.Columns.Add("EmployeeName", typeof(string));
            dt_ser.Columns.Add("Salary", typeof(decimal));
            dt_ser.Columns.Add("Addition", typeof(decimal));
            dt_ser.Columns.Add("Deduction", typeof(decimal));
            dt_ser.Columns.Add("IncentiveAmount", typeof(decimal));
            ...
            {
                DataSet ds = obj_trans.GetEmployeeSalaryDetails(Convert.ToInt32(drpMonth.SelectedValue),Convert.ToInt32(drpYear.SelectedValue));
                if (ds.Tables[0].Rows.Count > 0)
                {
                    rpt_Item_list.DataSource = ds.Tables[0];
                    rpt_Item_list.DataBind();
                    txtTotal.Text = ds.Tables[1].Rows[0][0].ToString();
                }
            }
        ...
        {
            int res = obj_trans.Delete_EmployeeSalaryProcess(Convert.ToInt32(hdn_id.Value),Convert.ToInt32(hdn_user_id.Value));
            if (res > 0)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            DataTable dt = obj_common.Get_Code(34);
            if (dt.Rows.Count > 0)
        ...
        public void previlage_action_check()
        {
            try
            {
                ...
                {
                    DataTable dt = obj_common.Action_Previlage_Validation(34, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        hdn_print.Value = dt.Rows[5][1].ToString();
                    }
                    btn_save.Visible = hdn_add.Value == "0" ? false : true;
                }
                else
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(34, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
