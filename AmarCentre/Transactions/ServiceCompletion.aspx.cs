...
namespace AmarCentre.Transactions
{
    public partial class ServiceCompletion : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size, string filter, string column, string order)
        {
            ...
            if (page_number == 1 && filter == "" && drpStatus.SelectedValue == "2" )
            {
                dt = obj_trans.Get_List_Invoice_ForSC_Custm(page_size, Convert.ToInt32(hdn_user_id.Value),
            ...
            {
                dt = obj_trans.Get_List_Invoice_ForServiceCompletion(page_number, page_size, filter, column, order,
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_trans.Get_InvDetail_ServiceCompletion(Convert.ToInt32(hdn_rpt_id.Value), Convert.ToInt32(hdn_user_id.Value));
            DataTable dt1 = ds.Tables[0];/*invoic*/
            DataTable dt_ser = ds.Tables[1];/* Detail*/

            lbl_Code.Text = dt1.Rows[0]["Code"].ToString();
            lblcustomer_name.Text = dt1.Rows[0]["CustomerName"].ToString();
            hdn_invId.Value = dt1.Rows[0]["Id"].ToString();
            hdn_invStatus.Value= dt1.Rows[0]["Status"].ToString();
        ...
        {
            HiddenField hdnDescpCount = (HiddenField)e.Item.FindControl("hdnDescpCount");

            HtmlTableRow trmainlist = (HtmlTableRow)e.Item.FindControl("trmainlist");
            trmainlist.BgColor = hdnDescpCount.Value == "0" ? "White" : "Yellow";
        }
        ...
        public void SaveServiceCompletion(DataSet dt_deatils)
        {
            ...
            else
            {
                ...
                if (res > 0)
                {
                    fill_ServiceList(Convert.ToInt32(hdn_invId.Value));
                    Clear();
        ...
        protected void btn_FinalSave_OnClick(object sender, EventArgs e)
        {
            ...
            if (dt_deatils.Tables[0].Rows.Count > 0)
            {
                ...
                if (serviceamt < expenseamt && expenseamt > 0 )
                {
                    chk = 1;
               ...
               if (dt_deatils.Tables[0].Rows[0]["PayModeId"].ToString() == "3")
                {
                    ...
                    if (Convert.ToDecimal(dt.Rows[0]["Balance"]) < expenseamt  && expenseamt>0)
                    {
                        chk = 1;
               ...
               if (dt_deatils.Tables[0].Rows[0]["PayModeId"].ToString() == "7")
                {
                    int VendorId = Convert.ToInt32(dt_deatils.Tables[0].Rows[0]["VendorId"]);
                    DataTable dt = obj_master.Edit_Vendor(VendorId).Tables[0];
                    if (Convert.ToDecimal(dt.Rows[0]["Balance"])*-1 < expenseamt && expenseamt > 0 )
                    {
                        chk = 1;
                        ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Confirm()", true);
                    }
        ...
        public DataSet fill_Detail()
        {
                ...
                && drpNewPayMode.SelectedValue!="" )
            {
              ...
              txtNewPaidAmount.Text==""?0:  Convert.ToDecimal(txtNewPaidAmount.Text), txtVendorCommissionOut.Text == "" ? 0 : Convert.ToDecimal(txtVendorCommissionOut.Text));
            }
        ...
        public void fill_ServiceList(int InvoiceId)
        {
            DataSet ds = obj_trans.Get_InvDetail_ServiceCompletion(InvoiceId, Convert.ToInt32(hdn_user_id.Value));
            DataTable dt1 = ds.Tables[0];/*invoic*/
            DataTable dt_ser = ds.Tables[1];/* Detail*/
        ...
        protected void drpNewPayMode_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            rqdaccOut.Enabled = true;
            txtNewPaidAmount.ReadOnly = false;
        ...
        protected void rptServiceCompletionEdit_OnItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                Button btnDebitnote = (Button)e.Item.FindControl("btnDebitNote");
                btnDebitnote.Visible = hdn_debitnote.Value == "0" ? false : true;

                Button btnEdit = (Button)e.Item.FindControl("btnServiceCompletionEdit");
                HiddenField hdnstatus = (HiddenField)e.Item.FindControl("hdnStatus");
                //if (hdnstatus.Value == "1")
                //    btnDebitnote.Visible = btnServiceCompletionDelete.Visible = btnEdit.Visible = false;
            }
        ...
        protected void btn_saveAddtnl_OnClick(object sender, EventArgs e)
        {
            ...
            if (rptAddtnlExpense.Items.Count > 0 || (drpAddExpense.SelectedValue != "" && txtAddAmt.Text != "" && txtAddVat.Text != "" && drpAddVendor.SelectedValue != ""
        ...
        protected void btnDebitNote_Click(object sender, EventArgs e)
        {
            ...
            RepeaterItem itemrp = (RepeaterItem)sendercontrol.NamingContainer;
            HiddenField hdnSerCompletionId = (HiddenField)itemrp.FindControl("hdnSerCompletionId");
            DataSet ds = obj_trans.DebitNoteServiceCompletion(Convert.ToInt32(hdnSerCompletionId.Value));
            DataTable dtSerCom = ds.Tables[0];
            DataTable dtSerComDetail = ds.Tables[1];
            DataTable dtSerComTransDetail = ds.Tables[2];
            hdn_id.Value = dtSerCom.Rows[0]["Id"].ToString();
            hdndnscid.Value = hdnSerCompletionId.Value;
            hdn_InvDetailId.Value = dtSerCom.Rows[0]["InvDetailId"].ToString();
            hdndninvid.Value = dtSerCom.Rows[0]["InvId"].ToString();
            hdnSingleamount.Value = dtSerCom.Rows[0]["AfterDiscount_PriceWitTax"].ToString();
            DataTable dt = obj_common.Get_Code(145);
            if (dt.Rows.Count > 0)
                txtDNCode.Text = dt.Rows[0][0].ToString();
            dn_date.DbSelectedDate = DateTime.Now;
            txtdnscno.Text = dtSerCom.Rows[0]["code"].ToString();
            txtDNInvNo.Text = dtSerCom.Rows[0]["invoiceno"].ToString();
            txtDNServiceName.Text = dtSerCom.Rows[0]["servicename"].ToString();
            txtdnqty.Text = dtSerCom.Rows[0]["Quantity"].ToString();
            txt_amtSQty.Text = dtSerCom.Rows[0]["AmtForSingleQty"].ToString();
            txt_totAmt.Text = dtSerCom.Rows[0]["TotalAmount"].ToString();
        ...
        public int SaveDebitNote()
        {
            ...
            if (dt_deatils.Rows.Count > 0)
            {
                ...
                res = obj_trans.Insert_UpdateDebitnote(0, dn_date.SelectedDate,
                    Convert.ToInt32(hdndninvid.Value), Convert.ToInt32(hdndnscid.Value), Convert.ToDecimal(txtdnqty.Text), "",
            ...
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        hdn_debitnote.Value = "0";// dt.Rows[7][1].ToString(); not proper
                        //hdn_debitnote.Value = "1";
