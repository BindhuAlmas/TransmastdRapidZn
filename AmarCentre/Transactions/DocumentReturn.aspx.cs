...
{
    public partial class DocumentReturn : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_trans.ListDocReturn(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_trans.DocReturnExcel();
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_trans.EditDocReturn(Convert.ToInt32(hdn_rpt_id.Value));
            ...
            DataSet ds2 = obj_trans.EditDocReturnDoc(Convert.ToInt32(hdn_rpt_id.Value), Convert.ToInt32(Drp_Cust.SelectedValue));
        ...
        protected void btn_save_OnClick(object sender, EventArgs e)
        {
            ...
            {
                int res = obj_trans.insertupdateDocReturn(Convert.ToInt32(hdn_id.Value), DateTime.ParseExact(CalDate(on_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
        ...
        protected void btn_save_print_OnClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (res > 0)
                {
                    ...
                    lbl_msg.Text = "Saved Successfully !..";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDiv();", true);
                    string url = "../Reports/DocumentReturnPrint.aspx?id=" + res;
        ...
        {
            int res = obj_trans.deleteDocReturn(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            string url = "../Reports/DocumentReturnPrint.aspx?id=" + Convert.ToInt32(hdn_id.Value);
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "NewWindow", "window.open('" + url + "','_blank','height=600,width=900,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=no,titlebar=no' );", true);
        }
        ...
        {
            DataSet ds = obj_trans.GetForDocReturn(Drp_Cust.SelectedValue == "" ? 0 : Convert.ToInt32(Drp_Cust.SelectedValue));
        ...
        {
            DataTable dt = obj_common.Get_Code(73);
            if (dt.Rows.Count > 0)
