...
namespace AmarCentre.Transactions
{
    public partial class PaymentVoucher : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = BalVoucher.List_PaymentVoucher(page_number, page_size, filter, column, order, Convert.ToInt32(hdn_user_id.Value));
        ...
        {
            DataTable dt = BalVoucher.Get_List_PaymentVoucher_Excel();
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "PaymentVoucher");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            pnl_add.Visible = true;
            UCPV.UCPageLoad(1, Convert.ToInt32(hdn_rpt_id.Value), txt_search.Text, Convert.ToInt32(drp_count.SelectedValue));
        ...
        {
            pnl_add.Visible = true;
            UCPV.UCPageLoad(1, 0, txt_search.Text, Convert.ToInt32(drp_count.SelectedValue));
            Upd_Add_Panel.Update();
        }
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(24, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
