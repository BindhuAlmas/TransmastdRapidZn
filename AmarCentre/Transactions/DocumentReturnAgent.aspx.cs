...
{
    public partial class DocumentReturnAgent : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_trans.Get_List_DocReturnAgent(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_trans.excel_get_DocReAgenttype();
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_trans.Edit_DocReturnAgent(Convert.ToInt32(hdn_rpt_id.Value));
            ...
            DataSet ds2 = obj_trans.Edit_DocReturnAgent_Doc(Convert.ToInt32(hdn_rpt_id.Value), Convert.ToInt32(Drp_Cust.SelectedValue));
            DataTable dt2 = ds2.Tables[0];
            Rpt_Doc.DataSource = dt2;
            Rpt_Doc.DataBind();
        ...
        protected void btn_save_OnClick(object sender, EventArgs e)
        {
            ...
            {
                int res = obj_trans.Insert_Update_DocumentreturnAgent(Convert.ToInt32(hdn_id.Value), DateTime.ParseExact(CalDate(on_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
        ...
        protected void btn_save_print_OnClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (res > 0)
                {
                    ...
                    lbl_msg.Text = "Saved Successfully !..";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDiv();", true);
                    string url = "../Reports/DocumentAgentPrint.aspx?id=" + res;
        ...
        {
            int res = obj_trans.Delete_DocReturnAgent(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            string url = "../Reports/DocumentAgentPrint.aspx?id=" + Convert.ToInt32(hdn_id.Value);
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "NewWindow", "window.open('" + url + "','_blank','height=600,width=900,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=no,titlebar=no' );", true);
        }
                ...
                foreach (RepeaterItem itm in Rpt_Doc.Items)
                {
                    ...
                    if (chk_sel.Checked)
                        dt_doc.Rows.Add(Convert.ToInt32(hdn_D_id.Value), Convert.ToInt32(hdn_doc_id.Value),
                            lbl_num.Text, lbl_remark.Text, v_frm.Value == "" ? null : v_frm.Value, v_to.Value == "" ? null : v_to.Value, lbl_newremark.Text, hdn_file.Value);
                }
            }
        ...
        protected void drp_Cust_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            DataSet ds = obj_trans.Get_DocReturnAgent(Drp_Cust.SelectedValue == "" ? 0 : Convert.ToInt32(Drp_Cust.SelectedValue));
            Rpt_Doc.DataSource = ds.Tables[0];
            Rpt_Doc.DataBind();
        ...
        {
            DataTable dt = obj_common.Get_Code(87);
            if (dt.Rows.Count > 0)
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(73, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    DataTable dt = obj_common.Action_Previlage_Validation(73, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
