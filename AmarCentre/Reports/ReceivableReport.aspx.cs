...
{
    public partial class ReceivableReport : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            DataSet ds = obj_report.GetReceivableReport(page_number, page_size, txt_search.Text, drptype.SelectedValue == "" ? 1 : Convert.ToInt32
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                InvoiceAmount.Text = ds.Tables[1].Rows[0]["InvoiceAmount"].ToString();
                PaidAmount.Text = ds.Tables[1].Rows[0]["PaidAmount"].ToString();
                GovtCharge.Text = ds.Tables[1].Rows[0]["GovtCharge"].ToString();
                ServiceCharge.Text = ds.Tables[1].Rows[0]["ServiceCharge"].ToString();
                Tax.Text = ds.Tables[1].Rows[0]["Tax"].ToString();
                Receivable.Text = ds.Tables[1].Rows[0]["Receivable"].ToString();
                GovtChargeToCollect.Text = ds.Tables[1].Rows[0]["GovtChargeToCollect"].ToString();
                OBToCollect.Text = ds.Tables[1].Rows[0]["OBToCollect"].ToString();
                DiffInGovtChargeCollected.Text = ds.Tables[1].Rows[0]["DiffInGovtChargeCollected"].ToString();
            }
            else
            {
                ...
                InvoiceAmount.Text = PaidAmount.Text = GovtCharge.Text = ServiceCharge.Text = "";
                Tax.Text = Receivable.Text = GovtChargeToCollect.Text = OBToCollect.Text= DiffInGovtChargeCollected.Text= "";
            }
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet ds = obj_report.GetReceivableExcel(txt_search.Text, drptype.SelectedValue == "" ? 1 : Convert.ToInt32(drptype.SelectedValue));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=ReceivableReport.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(123, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
