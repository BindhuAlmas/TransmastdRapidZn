...
{
    public partial class DetailedDebitor : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            if (dtgen.Rows[0]["DebitorsReportFormat"].ToString() == "2")
                ds = obj_report.Detaileddebitors_ReportF2(page_number, page_size, txtFromDate.SelectedDate, txtToDate.SelectedDate);
            else
                ds = obj_report.Detaileddebitors_Report(page_number, page_size, txtFromDate.SelectedDate, txtToDate.SelectedDate);
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet ds = new DataSet();
            if (dtgen.Rows[0]["DebitorsReportFormat"].ToString() == "2")
                ds = obj_report.Detaileddebitors_ExcelF2(txtFromDate.SelectedDate, txtToDate.SelectedDate);
            else
                ds = obj_report.Detaileddebitors_Excel(txtFromDate.SelectedDate, txtToDate.SelectedDate);
        ...
        {
            string url = "../Reports/DetailedDebitPdf.aspx?FromDate=" + txtFromDate.SelectedDate
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(95, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
