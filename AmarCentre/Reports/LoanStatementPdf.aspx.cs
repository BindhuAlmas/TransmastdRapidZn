...
{
    public partial class LoanStatementPdf : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataSet ds = obj_report.LoanStatementExcel(FromDate, ToDate, LoanId);
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "inline;filename=LoanStatement.pdf");
            ...
            PdfPCell HT00 = new PdfPCell(new Phrase("Loan Statement", new Font(Font.FontFamily.TIMES_ROMAN, 14, Font.BOLD)));
            ...
            sub00 = new PdfPCell(new Phrase("Name : " + dtEmp.Rows[0]["Loan Name"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            sub00.Border = 0;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                float[] detailsTableWidthsdet = new float[] { 5f, 8f, 25f, 25f,15f,9f,9f,  9f };
                ...
                detailHead = new PdfPCell(new Phrase("Description", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                detailHead.HorizontalAlignment = Element.ALIGN_CENTER;
                detailsTable.AddCell(detailHead);
                ...
                int i = 0;
                decimal Bal = Convert.ToDecimal(dtEmp.Rows[0]["Opening Balance"]);
                foreach (DataRow rows in dt.Rows)
                {
                    ...
                    try
                    {
                        detailCell = new PdfPCell(new Phrase(rows["Description"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
                    ...
                    try
                    {
                        ...
                        Bal = Bal + Convert.ToDecimal(rows["Debit"]);
                    ...
                    try
                    {
                        ...
                        detailCell.HorizontalAlignment = PdfPCell.ALIGN_RIGHT;
                        detailsTable.AddCell(detailCell);
                        Bal = Bal - Convert.ToDecimal(rows["Credit"]);
                    }
                    catch (Exception ee)
                    {
                        detailsTable.AddCell(emptyDetail);
                    ...
                    try
                    {
                        detailCell = new PdfPCell(new Phrase(Bal.ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
                        detailCell.HorizontalAlignment = PdfPCell.ALIGN_RIGHT;
                        detailsTable.AddCell(detailCell);
                    }
                    catch (Exception ee)
