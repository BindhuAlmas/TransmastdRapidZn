...
namespace AmarCentre.Reports
{
    public partial class InvoiceReport : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.InvoiceReport(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate, page_number, page_size,
                drpEmployee.SelectedValue==""?(int?)null:Convert.ToInt32(drpEmployee.SelectedValue),
                drpPayment.SelectedValue==""?0:Convert.ToInt32(drpPayment.SelectedValue),
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTotalInvoiceAmt.Text = ds.Tables[1].Rows[0]["TotalInvoiceAmount"].ToString();
                lblTotalGovtFee.Text = ds.Tables[1].Rows[0]["TotalGovtfee"].ToString();
                lblTotalServiceCharge.Text = ds.Tables[1].Rows[0]["TotalServicecharge"].ToString();
                lblTotalTax.Text = ds.Tables[1].Rows[0]["TotalTax"].ToString();
                lblTotalPaid.Text = ds.Tables[1].Rows[0]["TotalPaidAmount"].ToString();
                lblTotalBal.Text = ds.Tables[1].Rows[0]["TotalBalance"].ToString();
                lblTotalExpPaid.Text = ds.Tables[1].Rows[0]["TotalExpensePaid"].ToString();
                lblTotalExpPayable.Text = ds.Tables[1].Rows[0]["TotalExpensePayable"].ToString();
                lblTotalProfit.Text = ds.Tables[1].Rows[0]["TotalProfit"].ToString();
                lblvendrcommission.Text = ds.Tables[1].Rows[0]["VendorCommission"].ToString();

                lblCommission.Text = ds.Tables[1].Rows[0]["TotalCommission"].ToString();
                lblReceivedTax.Text = ds.Tables[1].Rows[0]["TotalReceivedTax"].ToString();
                lblPaidTax.Text = ds.Tables[1].Rows[0]["TotalPaidTax"].ToString();
                lblTaxPayable.Text = ds.Tables[1].Rows[0]["TotalTaxPayable"].ToString();
            }
            else
            {
                ...
                lblTotalInvoiceAmt.Text = "";
                lblTotalPaid.Text = "";
                lblTotalBal.Text = "";
                lblTotalExpPaid.Text = "";
                lblTotalExpPayable.Text = "";
                lblTotalProfit.Text = "";
        ...
        {
            DataSet ds = obj_report.InvoiceReportExcel(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
                 drpEmployee.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpEmployee.SelectedValue),
                 drpPayment.SelectedValue == "" ? 0 : Convert.ToInt32(drpPayment.SelectedValue),
                 drpCustomer.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpCustomer.SelectedValue),
                 drpagent.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpagent.SelectedValue));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=InvoiceReport.xls");
        ...
        protected void btnsummaryOnClick(object sender, EventArgs e)
        {
            string url = "../Reports/InvoiceSummary.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
        + "&ToDate=" + txt_reg_to_date.SelectedDate+"&AgentId=" + drpagent.SelectedValue;
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(42, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
