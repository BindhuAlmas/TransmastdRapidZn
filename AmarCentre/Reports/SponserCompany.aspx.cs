...
{
    public partial class SponserCompany : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.SponserCompanylist(drpSponser.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpSponser.SelectedValue), page_number, page_size);
        ...
        {
            DataSet ds = obj_report.SponserCompanyExcel(drpSponser.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpSponser.SelectedValue));
            DataTable dt = ds.Tables[0];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=SponserCompanyExcel.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(107, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
