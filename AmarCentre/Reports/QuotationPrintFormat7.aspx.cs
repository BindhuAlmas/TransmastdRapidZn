...
{
    public partial class QuotationPrintFormat7 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            PdfPCell sub21add = new PdfPCell(new Phrase("Address ", new Font(Font.FontFamily.UNDEFINED, 10, Font.BOLD)));
            sub21add.Border = 0;
            ...
            PdfPCell sub122add = new PdfPCell(new Phrase(dt_cust.Rows[0]["Address"].ToString(), new Font(Font.FontFamily.UNDEFINED, 10, Font.NORMAL)));
            sub122add.Border = 0;
            sub122add.Colspan = 5;
            ...
            if (dt_invD.Rows.Count > 0)
            {
                ...
                float[] widthsdet = new float[] { 4f, 25f, 10f, 8f,  9f };
                ...
                foreach (DataRow rows in dt_invD.Rows)
                {
                    ...
                    try
                    {
                        ...
                        //    (decimal.TryParse(rows["Price"]?.ToString(), out var expense) ? expense : 0) +
                        //    (decimal.TryParse(rows["TotalServiceChargeafterDiscount"]?.ToString(), out var serviceCharge) ? serviceCharge : 0)
                        //).ToString("F2"),
                        //new Font(Font.FontFamily.UNDEFINED, 10, Font.NORMAL)));
                    ...
                    try
                    {
                        ...
                        //F = F+ Convert.ToDecimal(rows["Quantity"]);
                    }
                    catch (Exception eee)
                    {
                        emp_details.AddCell("");
                ...
                if (dt_inv.Rows[0]["Grand_Total"].ToString() != "0.00")
                {
