...
namespace AmarCentre.Reports
{
    public partial class LoanStatement : System.Web.UI.Page
    {
        ...
        {
            drpLoan.Items.Clear();
            drpLoan.DataSource = obj_report.Drp_Loan();
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            DataSet ds = obj_report.LoanStatement(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
                Convert.ToInt32(drpLoan.SelectedValue), page_number, page_size);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblLoanName.Text = ds.Tables[1].Rows[0]["LoanName"].ToString();
                ...
                lblReceivable.Text = ds.Tables[1].Rows[0]["Receivable"].ToString();
                lblPayable.Text = ds.Tables[1].Rows[0]["Payable"].ToString();

                if (Convert.ToDecimal(lblPayable.Text) > 0)
                {
                    tdReceivable.Visible = thReceivable.Visible = false;
                    tdPayable.Visible = thPayable.Visible = true;
                }
                else
                {
                    tdReceivable.Visible = thReceivable.Visible = true;
                    tdPayable.Visible = thPayable.Visible = false;
                }
            ...
            else
            {
                ...
                tdReceivable.Visible=thReceivable.Visible=tdPayable.Visible = thPayable.Visible = false;
        ...
        {
            DataSet ds = obj_report.LoanStatementExcel(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
                Convert.ToInt32(drpLoan.SelectedValue));
            DataTable dtSum = ds.Tables[0];
            DataTable dt = ds.Tables[1];
            if (Convert.ToDecimal(dtSum.Rows[0]["Payable"].ToString()) > 0)
                dtSum.Columns.Remove("Receivable");
            else
                dtSum.Columns.Remove("Payable");
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=LoanStatement.xls");
        ...
        {
            string url = "../Reports/LoanStatementPdf.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
        + "&ToDate=" + txt_reg_to_date.SelectedDate + "&LoanId=" + Convert.ToInt32(drpLoan.SelectedValue);
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(45, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
