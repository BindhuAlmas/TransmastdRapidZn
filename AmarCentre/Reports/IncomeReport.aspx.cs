...
namespace AmarCentre.Reports
{
    public partial class IncomeReport : System.Web.UI.Page
    {
        ...
        {
            drp_income.Items.Clear();
            drp_income.DataSource = obj_report.Drp_Income_RVOther();
            drp_income.DataTextField = "text";
            drp_income.DataValueField = "value";
            drp_income.DataBind();
        }

        public DataTable fill_get_income()
        {
            DataTable dtstatus = new DataTable();
            dtstatus.Columns.Add("StatusId");
            if (dtstatus.Rows.Count > 0)
                dtstatus.Rows.Clear();
            foreach (RadComboBoxItem item in drp_income.CheckedItems)
            {
        ...
        {
            DataTable dt_income = fill_get_income();

             DataSet ds  =  obj_report.Get_income_Report(txt_reg_Frm_date.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txt_reg_Frm_date), "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null,
                ...
                dt_income, page_number, page_size, 4);
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet ds = obj_report.Get_income_Excel(txt_reg_Frm_date.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txt_reg_Frm_date), "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null,
                txt_reg_to_date.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txt_reg_to_date), "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null, dt_income
                ,4);
            DataTable dt = ds.Tables[0];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=IncomeReport.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(27, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
