...
{
    public partial class DiscountReportNav : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.DiscountShortlist(txt_reg_Frm_date.SelectedDate,
                txt_reg_to_date.SelectedDate, drpCustomer.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpCustomer.SelectedValue), page_number, page_size);
        ...
        {
            DataSet ds = obj_report.DiscountShortlistExcel(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate, drpCustomer.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpCustomer.SelectedValue));
            DataTable dt = ds.Tables[0];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=DiscountShortlistExcel.xls");
        ...
        {
            string url = "../Reports/DiscountReport.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
                + "&ToDate=" + txt_reg_to_date.SelectedDate + "&Cus=" + (drpCustomer.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpCustomer.SelectedValue));
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(100, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
