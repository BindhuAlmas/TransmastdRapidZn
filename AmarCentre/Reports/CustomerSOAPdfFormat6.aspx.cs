...
{
    public partial class CustomerSOAPdfFormat6 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            cust21 = new PdfPCell(new Phrase("TRN", new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            cust21.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
            tbl_basic.AddCell(cust21);
            cust1 = new PdfPCell(new Phrase(dtCustomer.Rows[0]["CustomerTRN"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            cust1.HorizontalAlignment = PdfPCell.ALIGN_LEFT;
            tbl_basic.AddCell(cust1);

            PdfPCell CustOut = new PdfPCell(new Phrase("Outstanding Amount", new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            CustOut.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
            ...
            PdfPCell CustAdv = new PdfPCell(new Phrase("Advance Payment", new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            CustAdv.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
            tbl_basic.AddCell(CustAdv);
            PdfPCell CustAdv1 = new PdfPCell(new Phrase(dtCustomer.Rows[0]["Payable"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL)));
            ...
            {
                // Changed from 9 to 10 columns - added WorkStatus after Qty
                PdfPTable emp_details = new PdfPTable(10);
                emp_details.DefaultCell.Padding = 4;
                emp_details.WidthPercentage = 100;
                float[] widths = new float[] { 6f, 10f, 9f, 21f, 14f, 5f, 12f, 11f, 11f, 12f };
                ...
                // NEW: Work Status header - added after Qty
                PdfPCell ws = new PdfPCell(new Phrase("Work Status", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                ws.HorizontalAlignment = Element.ALIGN_CENTER;
                emp_details.AddCell(ws);
                ...
                foreach (DataRow rows in dtDetails.Rows)
                {
                    ...
                    foreach (DataRow r in dh.Rows)
                    {
                        ...
                        // NEW: Work Status - color coded
                        string workStatus = r["WorkStatus"].ToString();
                        Font workStatusFont;
                        if (workStatus == "Completed")
                            workStatusFont = new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL);
                        else if (workStatus.Contains("Pending"))
                            workStatusFont = new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL);
                        else
                            workStatusFont = new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL);

                        PdfPCell wsCell = new PdfPCell(new Phrase(workStatus, workStatusFont));
                        wsCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
                        wsCell.MinimumHeight = 15f;
                        emp_details.AddCell(wsCell);

                        // Empty border cell for Amount/Paid/Outstanding (only shown on first sub-row)
                        ...
                        // Paid Amount (first sub-row only)
                        if (rr == 0)
                        {
                            try
                            {
                        ...
                        // Outstanding (first sub-row only)
                        if (rr == 0)
                        {
                            try
                            {
                ...
                // --- Total Row ---
                // Now colspan = 7 (was 6) because we have one extra column (WorkStatus)
