...
{
    public partial class SalesOrderFormat2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (dt_invD.Rows.Count > 0)
            {
                ...
                if (dt_inv.Rows[0]["InvoiceType"].ToString() == "1")
                {
                    ...
                    float[] widthsdet = new float[] { 4f, 30f, 13f, 9f, 8f, 8f,9f,6f, 8f, 7f, 11f };
                    ...
                    emp_details.AddCell(DetailH04);
                    ph1 = new Phrase();
                    ph1.Add(new Chunk("Vat %", new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.BOLD)));
                    ...
                    foreach (DataRow rows in dt_invD.Rows)
                    {
                        ...
                        try
                        {
                            if (dt_inv.Rows[0]["DisplDiscount"].ToString() == "1" & rows["AfterDiscount_TotalSO"].ToString() != "")
                            {
                                ...
                                decimal ca = Convert.ToDecimal(rows["ChargedAmount"]) + Convert.ToDecimal(rows["AfterDiscount_TotalSO"]);
                    ...
                    emp_details.AddCell(tot1);
                     ...
                     caout = (dt_inv.Rows[0]["DisplDiscount"].ToString() == "1" & dt_sum.Rows[0]["AfterDiscount_Total"].ToString() != "") ? Convert.ToDecimal(dt_inv.Rows[0]["ChargedAmount"]) + Convert.ToDecimal(dt_sum.Rows[0]["AfterDiscount_Total"]) :
                        Convert.ToDecimal(dt_inv.Rows[0]["ChargedAmount"]) + Convert.ToDecimal(dt_sum.Rows[0]["Total"])  ;
                    ...
                    foreach (DataRow rows in dt_invD.Rows)
                    {
                        ...
                        try
                        {
                            if (dt_inv.Rows[0]["DisplDiscount"].ToString() == "1" & rows["AfterDiscount_TotalSO"].ToString() != "")
                            {
                                ...
                                decimal ca = Chargedamt + Convert.ToDecimal(rows["AfterDiscount_TotalSO"]);
                ...
                //PdfPCell totw1wcc = new PdfPCell(new Phrase("Received :", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                //totw1wcc.Border = 0;
