...
namespace AmarCentre.Reports
{
    public partial class CustomerBalance : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.CustomerBalanceList(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                TotalReceivedAmount.Text = ds.Tables[1].Rows[0]["TotalReceivedAmount"].ToString();
                TotalWorkCompleted.Text = ds.Tables[1].Rows[0]["TotalWorkCompleted"].ToString();
                TotalReceivable.Text = ds.Tables[1].Rows[0]["TotalReceivable"].ToString();
                TotalPayable.Text = ds.Tables[1].Rows[0]["TotalPayable"].ToString();
            }
            else
            {
        ...
        protected void lnkcust_Click(object sender, EventArgs e)
        {
            ...
            string url = "../Reports/CustomerBalanceDetail.aspx?CustomerId=" + Convert.ToInt32(hdnCustomerId.Value) + "&ToDate=" + txt_reg_to_date.SelectedDate + "&FromDate=" + txt_reg_Frm_date.SelectedDate ;
        ...
        {
            DataSet ds = obj_report.CustomerBalancePdfExcel(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate);
            DataTable dt = ds.Tables[0];
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=CustomerBalance.xls");
        ...
        {
            string url = "../Reports/CustomerBalancePdf.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
        + "&ToDate=" + txt_reg_to_date.SelectedDate;
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "NewWindow", "window.open('" + url + "','_blank','height=600,width=900,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=no,titlebar=no' );", true);
        }
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(79, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
