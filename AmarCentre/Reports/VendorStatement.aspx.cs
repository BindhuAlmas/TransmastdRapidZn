...
{
    public partial class VendorStatement : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            DataSet ds = obj_report.VendorStatementVersion2(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
                Convert.ToInt32(drpVendor.SelectedValue), page_number, page_size);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblBalance.Text = ds.Tables[0].Rows[0]["Payable"].ToString();

                lbl_page_info.Text = "Showing Results " + dt.Rows[0]["start_num"].ToString() + " - " + dt.Rows[dt.Rows.Count - 1]["Slno"].ToString() + " Out of " + dt.Rows[0]["current_count"].ToString() + " Records";
        ...
        {
            DataSet ds = obj_report.VendorStatementExcelVersion2(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate,
                Convert.ToInt32(drpVendor.SelectedValue));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=VendorStatement.xls");
        ...
        protected void btnPdfOnClick(object sender, EventArgs e)
        {
            ...
            string url = "";
            if (dt.Rows[0]["VendorStmtFormat"].ToString() == "1")
                url = "../Reports/Vendorpdf.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
           + "&ToDate=" + txt_reg_to_date.SelectedDate + "&VendorId=" + Convert.ToInt32(drpVendor.SelectedValue);
            else
                url = "../Reports/VendorPdfF2.aspx?FromDate=" + txt_reg_Frm_date.SelectedDate
      + "&ToDate=" + txt_reg_to_date.SelectedDate + "&VendorId=" + Convert.ToInt32(drpVendor.SelectedValue);
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(51, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
