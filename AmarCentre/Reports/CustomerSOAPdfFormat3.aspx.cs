...
{
    public partial class CustomerSOAPdfFormat3 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            catch (Exception ex) { FromDate = null; }

            DateTime? ToDate = null;
            try { ToDate = Convert.ToDateTime(Request.QueryString["ToDate"]); }
            catch (Exception ex) { ToDate = null; }

            DataSet ds = obj_report.CustomerSOAPrintFormat3(FromDate, ToDate, CustomerId, PaymentStatus, CompletionStatus);
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "inline;filename=CustomerSOA.pdf");
            ...
            if (dtDetails.Rows.Count > 0)
            {
                ...
                float[] detailsTableWidthsdet = new float[] { 5f, 15f,  20f, 50f,30f, 15f, 15f, 15f };
                ...
                detailHead = new PdfPCell(new Phrase("Ref No", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                detailHead.HorizontalAlignment = Element.ALIGN_CENTER;
                detailsTable.AddCell(detailHead);
                ...
                foreach (DataRow rows in dtDetails.Rows)
                {
                    ...
                    try
                    {
                        detailCell = new PdfPCell(new Phrase(rows["Descriptions"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
