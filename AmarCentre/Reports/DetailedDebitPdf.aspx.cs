...
{
    public partial class DetailedDebitPdf : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (dtgen.Rows[0]["DebitorsReportFormat"].ToString() == "2")
                ds = obj_report.Detaileddebitors_ExcelF2(FromDate, ToDate);
            else
                ds = obj_report.Detaileddebitors_Excel(FromDate, ToDate);
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "inline;filename=DetailedDebitReport.pdf");
            ...
            PdfPCell HT00 = new PdfPCell(new Phrase("Detailed Debtors Report ", new Font(Font.FontFamily.TIMES_ROMAN, 14, Font.BOLD)));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                float[] detailsTableWidthsdet = new float[] { 10f, 50f, 20f, 15f, 20f };
                ...
                foreach (DataRow rows in dt.Rows)
                {
                    try
                    {
                        dirExpenseCell = new PdfPCell(new Phrase(rows["Slno"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.NORMAL)));
