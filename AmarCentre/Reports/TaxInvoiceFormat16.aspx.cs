...
{
    public partial class TaxInvoiceFormat16 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            sub13.MinimumHeight = 40f;
            sub13.HorizontalAlignment = PdfPCell.ALIGN_LEFT;
            sub13.VerticalAlignment = PdfPCell.ALIGN_MIDDLE;
            table1.AddCell(sub13);

            lines = new PdfPCell(new Phrase("", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.NORMAL)));
            lines.Border = 0;
            lines.Colspan = 3;
            lines.HorizontalAlignment = Element.ALIGN_LEFT;
            table1.AddCell(lines);
            ...
            if (dt_invD.Rows.Count > 0)
            {
                if (dt_inv.Rows[0]["InvoiceType"].ToString() == "1")
                {
                    ...
                    ph1 = new Phrase();
                    ph1.Add(new Chunk("Particulars ", new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.BOLD)));
                    ...
                    foreach (DataRow rows in dt_invD.Rows)
                    {
                        ...
                        try
                        {
                            PdfPCell DT = new PdfPCell(new Phrase(rows["AmountNoDisTax"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.NORMAL)));
                    ...
                    summ = new PdfPCell(new Phrase(dt_sum.Rows[0]["TAmountNoDisTax"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                ...
                else if (dt_inv.Rows[0]["InvoiceType"].ToString() == "2")/*Normal Invoice Type*/
                {
                    ...
                    emp_details.DefaultCell.Padding = 4;
                    float[] widthsdet = new float[] { 4f, 25f, 10f, 5f, 9f,  9f };
                    emp_details.SetWidths(widthsdet);
