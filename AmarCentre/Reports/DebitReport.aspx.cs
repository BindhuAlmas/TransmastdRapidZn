...
{
    public partial class DebitReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                hdnformat.Value = dtgen.Rows[0]["DebitorsReportFormat"].ToString();
                lblheading.Text = hdnformat.Value == "2" ? "(Based on Work Completion)" : "";
                previlage_check();
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            if (hdnformat.Value == "2")
                ds = obj_report.Get_debitors_ReportFromat2(page_number, page_size,txt_search.Text,drptype.SelectedValue==""?1:Convert.ToInt32
                    (drptype.SelectedValue));
            else
                ds = obj_report.Get_debitors_Report(page_number, page_size, txt_search.Text, drptype.SelectedValue == "" ? 1 : Convert.ToInt32
                    (drptype.SelectedValue));
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            DataSet ds = new DataSet();
            if (hdnformat.Value == "2")
                ds = obj_report.Get_debitors_ExcelFromat2(drptype.SelectedValue == "" ? 1 : Convert.ToInt32(drptype.SelectedValue));
            else
                ds = obj_report.Get_debitors_Excel(drptype.SelectedValue == "" ? 1 : Convert.ToInt32(drptype.SelectedValue));
        ...
        {
            string url = "../Reports/DebitReportPdf.aspx?Ctype=" +( drptype.SelectedValue == "" ? "1" : drptype.SelectedValue ) ;
        ...
        protected void lnkcust_Click(object sender, EventArgs e)
        {
            ...
            string url = "../Reports/DebitReportDetail.aspx?CustomerId=" + Convert.ToInt32(hdnCustomerId.Value);
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "NewWindow", "window.open('" + url + "','_blank','height=600,width=900,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=no,titlebar=no' );", true);
        }
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(30, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
