...
namespace AmarCentre.Reports
{
    public partial class SummaryReport : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.BalanceSheet(page_number, page_size);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTotalCredit.Text = dt_sum.Rows[0]["TotalCredit"].ToString();
                lblTotalDebit.Text = dt_sum.Rows[0]["TotalDebit"].ToString();
            }
            else
            {
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet ds = obj_report.BalanceSheetExcel();
            DataTable dt = ds.Tables[0];
            DataTable dt_sum = ds.Tables[1];
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=BalanceSheet.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(41, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
