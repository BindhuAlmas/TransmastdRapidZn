...
namespace AmarCentre.Reports
{
    public partial class BankReconciliation : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            DataTable dt = obj_report.BankReconciliationStatement(DateTime.ParseExact(CalDate(txt_reg_Frm_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
                DateTime.ParseExact(CalDate(txt_reg_to_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
                Convert.ToInt32(drpBank.SelectedValue), page_number, page_size);
        ...
        {
            DataTable dt = obj_report.BankReconciliationStatementExcel(DateTime.ParseExact(CalDate(txt_reg_Frm_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
                DateTime.ParseExact(CalDate(txt_reg_to_date), "dd/MM/yyyy", CultureInfo.InvariantCulture),
                Convert.ToInt32(drpBank.SelectedValue));
            if (dt.Rows.Count > 0)
            {
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(61, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
