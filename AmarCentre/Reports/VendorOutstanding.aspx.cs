...
{
    public partial class VendorOutstanding : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.VendorOutstandingReport(page_number, page_size);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTotalPayable.Text = ds.Tables[1].Rows[0]["TotalPayable"].ToString();
                lblTotalReceivable.Text = ds.Tables[1].Rows[0]["TotalReceivable"].ToString();
                lblTotalOSAmount.Text = ds.Tables[1].Rows[0]["TotalOutstandingAmount"].ToString();
            }
            else
            {
        ...
        protected void btn_excel_OnClick(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=VendorOutstanding.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(50, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
