...
{
    public partial class ServiceCompletionpdf : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ServiceStatusId = Convert.ToInt32(Request.QueryString["ServiceStatusId"]);
            }
            catch (Exception xx)
            {
            ...
            DataTable dtService = (DataTable)Session["dt_service"];
            DataTable dtcust = (DataTable)Session["dt_cust"];
            DataTable dtbank = (DataTable)Session["dt_bank"];
            DataTable department = (DataTable)Session["dt_department"];

            DataTable dt = obj_report.Get_SC_Excel(FromDate, ToDate, dtService, dtcust, dtbank, department, employeeId, agentId, ServiceStatusId,"");

            Document document = new Document(PageSize.A4, 10f, 10f, 20f, 20f);

            HttpContext.Current.Response.ContentType = "application/pdf";
            HttpContext.Current.Response.AddHeader("content-disposition", "inline;filename=ServiceCompletionreport.pdf");
            ...
            PdfPCell cell1 = new PdfPCell(new Phrase("SERVICE COMPLETION REPORT", new Font(Font.FontFamily.TIMES_ROMAN, 14, Font.BOLD)));
            cell1.Border = 0;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                float[] widths = new float[] { 4f, 12f, 10f, 23f, 20f, 20f, 25f, 10f,12f };
                ...
                tp3 = new PdfPCell(new Phrase("Applicant", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                tp3.HorizontalAlignment = Element.ALIGN_CENTER;
                emp_details.AddCell(tp3);
                ...
                app = new PdfPCell(new Phrase("Approval  Status", new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD)));
                app.HorizontalAlignment = Element.ALIGN_CENTER;
                emp_details.AddCell(app);
                ...
                foreach (DataRow rows in dt.Rows)
                {
                    ...
                    try
                    {
                        PdfPCell TP = new PdfPCell(new Phrase(rows["Applicant"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
                        TP.HorizontalAlignment = PdfPCell.ALIGN_LEFT;
                        emp_details.AddCell(TP);
                    }
                    catch (Exception eee)
                    ...
                    try
                    {
                        PdfPCell TP = new PdfPCell(new Phrase(rows["Expense"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
                        TP.HorizontalAlignment = PdfPCell.ALIGN_LEFT;
                        emp_details.AddCell(TP);
                    }
                    catch (Exception eee)
                   ...
                   try
                    {
                        PdfPCell TP = new PdfPCell(new Phrase(rows["Status"].ToString(), new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.NORMAL)));
                        TP.HorizontalAlignment = PdfPCell.ALIGN_LEFT;
                        emp_details.AddCell(TP);
                    }
                    catch (Exception eee)
