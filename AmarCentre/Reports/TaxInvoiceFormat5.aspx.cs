...
{
    public partial class TaxInvoiceFormat5 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            float[] widths = new float[] { 21f, 25f, 38f, 15f, 25f };
            ...
            //if (dt_inv.Rows[0]["TokenNo"].ToString() != "")
            //{
            //    sub122add = new PdfPCell(new Phrase("Token No : ", new Font(Font.FontFamily.UNDEFINED, 10, Font.BOLD)));
            //    sub122add.Border = 0;
            //    sub122add.HorizontalAlignment = Element.ALIGN_RIGHT;
            //    Subhead.AddCell(sub122add);

            //    sub142 = new PdfPCell(new Phrase(dt_inv.Rows[0]["TokenNo"].ToString(), new Font(Font.FontFamily.UNDEFINED, 10, Font.NORMAL)));
            //    sub142.Border = 0;
            //    sub142.HorizontalAlignment = Element.ALIGN_LEFT;
            //    Subhead.AddCell(sub142);
            //}
            ...
            if (dt_invD.Rows.Count > 0)
            {
                ...
                if (dt_inv.Rows[0]["InvoiceType"].ToString() == "1")
                {
                    ...
                    float[] widthsdet = new float[] { 4f, 25f, 10f, 8f, 8f, 9f,9f, 5f, 9f };
                    ...
                    ph1.Add(new Chunk("اسم التطبيق", arbsmallbold));
                    PdfPCell DetailH03 = new PdfPCell(ph1);
                    DetailH03.HorizontalAlignment = Element.ALIGN_CENTER;
                    DetailH03.RunDirection = PdfWriter.RUN_DIRECTION_RTL;
                    emp_details.AddCell(DetailH03);
                    ...
                    foreach (DataRow rows in dt_invD.Rows)
                    {
                        ...
                        try
                        {
                            PdfPCell DT = new PdfPCell(new Phrase(rows["Expense"].ToString(), new Font(Font.FontFamily.UNDEFINED, 10, Font.NORMAL)));
                    ...
                    if (dt_inv.Rows[0]["InvoiceType"].ToString() == "2")
                    {
                        ...
                        foreach (DataRow rows in dt_invD.Rows)
                        {
                            ...
                            //try
                            //{
                            //    PdfPCell DT = new PdfPCell(new Phrase(rows["ApplicantName"].ToString(), new Font(Font.FontFamily.UNDEFINED, 9, Font.NORMAL)));
                            ...
                            try
                            {
                                PdfPCell DT = new PdfPCell(new Phrase(rows["Discount"].ToString(), new Font(Font.FontFamily.UNDEFINED, 9, Font.NORMAL)));
                                DT.HorizontalAlignment = PdfPCell.ALIGN_RIGHT;
                                emp_details.AddCell(DT);
                            ...
                            try
                            {
                                PdfPCell DT = new PdfPCell(new Phrase(rows["Fine"].ToString(), new Font(Font.FontFamily.UNDEFINED, 9, Font.NORMAL)));
                                DT.HorizontalAlignment = PdfPCell.ALIGN_RIGHT;
                                emp_details.AddCell(DT);
                            }
                            catch (Exception eee)
