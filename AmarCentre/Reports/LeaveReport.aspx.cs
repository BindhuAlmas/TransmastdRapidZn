...
namespace AmarCentre.Reports
{
    public partial class LeaveReport : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = obj_report.LeaveReport(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate, page_number, page_size,
        ...
        {
            DataSet ds = obj_report.LeaveReportExcel(txt_reg_Frm_date.SelectedDate, txt_reg_to_date.SelectedDate, 
                drpEmployee.SelectedValue == "" ? (int?)null : Convert.ToInt32(drpEmployee.SelectedValue)
                );
            DataTable dt = ds.Tables[0];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=LeaveReport.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(149, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
