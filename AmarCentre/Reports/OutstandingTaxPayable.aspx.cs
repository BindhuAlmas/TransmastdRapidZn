...
{
    public partial class OutstandingTaxPayable : System.Web.UI.Page
    {
        ...
        public void grid_fill(int page_number, int page_size)
        {
            ...
            DataSet ds = obj_report.OutstandingTaxPayable(txtFromDate.DbSelectedDate!=null? DateTime.ParseExact(CalDate(txtFromDate), "dd/MM/yyyy", CultureInfo.InvariantCulture):(DateTime?)null,
                txtToDate.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txtToDate), "dd/MM/yyyy", CultureInfo.InvariantCulture):(DateTime?)null, page_number, page_size);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTaxReceived.Text = ds.Tables[1].Rows[0]["ReceivedTax"].ToString();
                lblTaxPaid.Text = ds.Tables[1].Rows[0]["PaidTax"].ToString();
                lblOutstandingTaxPayable.Text = ds.Tables[1].Rows[0]["OutstandingTaxPayable"].ToString();
        ...
        {
            DataSet ds = obj_report.OutstandingTaxPayableExcel(txtFromDate.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txtFromDate), "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null,
                txtToDate.DbSelectedDate != null ? DateTime.ParseExact(CalDate(txtToDate), "dd/MM/yyyy", CultureInfo.InvariantCulture) : (DateTime?)null);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=OutstandingTaxPayable.xls");
        ...
        public void previlage_check()
        {
            try
            {
                ...
                {
                    int val = obj_common.Form_Previlage_Validation(65, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
