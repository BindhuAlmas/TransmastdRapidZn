...
namespace AmarCentre.Company
{
    public partial class Checklist : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_ListChecklist(page_number, page_size, filter);
        ...
        {
            DataTable dt = obj_master.Get_ListChecklistExcel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Checklist");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditChecklist(Convert.ToInt32(hdn_rpt_id.Value));
        ...
        {
            int res = obj_master.Insert_UpdateChecklist(Convert.ToInt32(hdn_id.Value), txt_name.Text,Convert.ToInt32(hdn_user_id.Value));
            if (res>0)
            {
        ...
        {
            int res = obj_master.DeleteChecklist(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(10, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(10, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
