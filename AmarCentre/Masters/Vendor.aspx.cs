...
namespace AmarCentre.Masters
{
    public partial class Vendor : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_List_Vendor(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_List_Vendor_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Vendor");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.Edit_Vendor(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            txt_name.Text = dt.Rows[0]["Name"].ToString();
            txt_address.Text = dt.Rows[0]["Addres"].ToString();
            txt_mob.Text = dt.Rows[0]["Mobile"].ToString();
            txt_email.Text = dt.Rows[0]["Email"].ToString();
            txt_trn.Text = dt.Rows[0]["TRN"].ToString();
            txtcommission.Text = dt.Rows[0]["VendorCommission"].ToString();
            chkIsAlsoCustomer.Checked =  Convert.ToBoolean(dt.Rows[0]["IsAlsoCustomer"]);
        ...
        {
            int res = obj_master.Delete_Vendor(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        public void Clear()
        {
            txt_name.Text =txtcommission.Text= "";
            txt_address.Text = "";
            txt_mob.Text = "";
            txt_email.Text = "";
        ...
        protected void btn_OB_OnClick(object sender, EventArgs e)
        {
            ...
            DataTable dt = obj_master.GetVendorOB(Convert.ToInt32(hdn_id.Value));
        ...
        {
            int res = obj_master.ClearVendorOB(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                drp_obType.ClearSelection();
                drp_obType.Text = "";
        ...
        {
            int res = obj_master.Update_OB_Vendor(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(drp_obType.SelectedValue),
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(6, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
