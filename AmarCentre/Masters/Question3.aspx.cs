...
namespace AmarCentre.Masters
{
    public partial class Question3 : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion3(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion3_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Question3");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditQuestion3(Convert.ToInt32(hdn_rpt_id.Value));
        ...
        {
            int res = obj_master.Insert_UpdateQuestion3(Convert.ToInt32(hdn_id.Value), txt_name.Text, txt_desc.Text,
        ...
        {
            int res = obj_master.DeleteQuestion3(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(76, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
