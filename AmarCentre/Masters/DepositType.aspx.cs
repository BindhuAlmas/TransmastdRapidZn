...
{
    public partial class DepositType : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.ListDepositType(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.ListExcelDepositType();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "DepositType");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditDepositType(Convert.ToInt32(hdn_rpt_id.Value));
        ...
        {
            int res = obj_master.Insert_UpdateDepositType(Convert.ToInt32(hdn_id.Value), txt_name.Text,Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
        ...
        {
            int res = obj_master.DeleteDepositType(Convert.ToInt32(hdn_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            txt_name.Text = "";
            hdn_id.Value = txtBalance.Text= "0";
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(105, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(105, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
