...
{
    public partial class Shareholder : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.ListShareholder(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.ListExcelShareholder();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "PartnerList");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditShareholder(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            txt_name.Text = dt.Rows[0]["Name"].ToString();
            txtshare.Text = dt.Rows[0]["SharePercentage"].ToString();
        ...
        {
            int res = obj_master.Insert_UpdateShareholder(Convert.ToInt32(hdn_id.Value), txt_name.Text, 
                Convert.ToDecimal(txtshare.Text), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
        ...
        {
            int res = obj_master.DeleteShareholder(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        protected void btn_OB_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet ds = obj_master.EditShareholder(Convert.ToInt32(hdn_id.Value));
            DataTable dt = ds.Tables[0];
        ...
        {
            int res = obj_master.Update_ShareHolder_OB(Convert.ToInt32(hdn_id.Value), Convert.ToDecimal(txt_open_bal.Text),
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(102, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(102, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
