...
namespace AmarCentre.Masters
{
    public partial class FixedAsset : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_ListFixedAsset(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_ListFixedAssetExcel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "FixedAsset");
        ...
        {
            int res = obj_master.Insert_UpdateFixedAsset(Convert.ToInt32(hdn_id.Value), txt_name.Text, txt_desc.Text,
                txtCurrentValue.Text == "" ? (decimal?)null : Convert.ToDecimal(txtCurrentValue.Text),
                Convert.ToInt32(hdn_user_id.Value),radOBDate.SelectedDate);
            if (res >0)
            {
        ...
        {
            int res = obj_master.DeleteFixedAsset(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
            ...
            {
                lblerrormsg.Text = "Unable to delete. Entry may be used in BankAccount Page !..";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDivError();", true);
            }
            pnl_add.Visible = false;
            Upd_Add_Panel.Update();
        ...
        protected void btnDispose_Click(object sender, EventArgs e)
        {
            ...
            radDisposaldate.DbSelectedDate = null;
            txtdisposalRemark.Text = "";
            upddisposal.Update();
        }

        protected void btndisposeSave_Click(object sender, EventArgs e)
        {
            int res = obj_master.DisposeFixedAsset(Convert.ToInt32(hdn_id.Value), 
                Convert.ToInt32(hdn_user_id.Value),radDisposaldate.SelectedDate,txtdisposalRemark.Text);
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        protected void btnHistory_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = obj_master.Assethistory(Convert.ToInt32(hdn_id.Value));
        ...
        public void Clear()
        {
            ...
            txtCurrentValue.ReadOnly = false;
            hdn_id.Value = "0";
            radOBDate.Enabled = true;
            radOBDate.DbSelectedDate = null;

            btn_delete.Visible =btnHistory.Visible=btnDispose.Visible= false;
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(48, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(48, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
                        hdn_add.Value = dt.Rows[0][1].ToString();
                        hdn_update.Value = dt.Rows[1][1].ToString();
                        hdn_delete.Value = dt.Rows[2][1].ToString();
                        hdnDispose.Value = dt.Rows[3][1].ToString();
                        hdnHistory.Value = dt.Rows[4][1].ToString();
                    }
                    btn_save.Visible = hdn_add.Value == "0" ? false : true;
                }
                else
