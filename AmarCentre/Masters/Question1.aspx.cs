...
namespace AmarCentre.Masters
{
    public partial class Question1 : System.Web.UI.Page
    {
        ...
        {
            drpdepartment.DataSource = obj_master.DrpLeadDepartment();
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion1(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion1_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Question1");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditQuestion1(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            txt_name.Text = dt.Rows[0]["Name"].ToString();
            txt_desc.Text = dt.Rows[0]["Description"].ToString();
            drpdepartment.SelectedValue = dt.Rows[0]["LeadDepartmentId"].ToString();
        ...
        {
            int res = obj_master.Insert_UpdateQuestion1(Convert.ToInt32(hdn_id.Value), txt_name.Text, txt_desc.Text,
                 Convert.ToInt32(hdn_user_id.Value),Convert.ToInt32(drpdepartment.SelectedValue));
            if (res > 0)
            {
        ...
        {
            int res = obj_master.DeleteQuestion1(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(137, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(137, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
