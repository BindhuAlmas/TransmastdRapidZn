...
namespace AmarCentre.Masters
{
    public partial class Question2 : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion2(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_ListQuestion2_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Question2");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.EditQuestion2(Convert.ToInt32(hdn_rpt_id.Value));
        ...
        {
            int res = obj_master.Insert_UpdateQuestion2(Convert.ToInt32(hdn_id.Value), txt_name.Text, txt_desc.Text,
        ...
        {
            int res = obj_master.DeleteQuestion2(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
