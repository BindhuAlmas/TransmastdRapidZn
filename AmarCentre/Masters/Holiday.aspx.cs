...
namespace AmarCentre.Masters
{
    public partial class Holiday : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_List_Holiday(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_List_Holiday_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "Holiday");
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.Edit_Holiday(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            txt_name.Text = dt.Rows[0]["Name"].ToString();
            holidayDate.DbSelectedDate = dt.Rows[0]["Date"].ToString();
        ...
        {
            int res = obj_master.Insert_Update_Holiday(Convert.ToInt32(hdn_id.Value), txt_name.Text,
                DateTime.ParseExact(CalDate(holidayDate), "dd/MM/yyyy", CultureInfo.InvariantCulture),txt_desc.Text,Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
        ...
        {
            int res = obj_master.Delete_Holiday(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
        ...
        {
            txt_name.Text = "";
            holidayDate.DbSelectedDate = null;
            txt_desc.Text = "";
            hdn_id.Value = "0";
