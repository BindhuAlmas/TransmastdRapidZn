...
{
    public partial class ServiceSubCategory : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_List_ServiceSubCategory(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_List_ServiceSubCategory_Excel();
            dt.Columns["Sl_No"].ColumnName = "Sl No.";
            if (dt.Rows.Count > 0)
            {
                StringWriter sw = obj_common.ExportToExcel(dt, "ServiceSubCategoryList");
        ...
        public void fill_ServiceCategory()
        {
            drpServiceCategory.Items.Clear();
            DataTable dt = obj_master.Drp_ServiceCategory();
            drpServiceCategory.DataSource = dt;
            drpServiceCategory.DataTextField = "Text";
            drpServiceCategory.DataValueField = "Value";
            drpServiceCategory.DataBind();
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.Edit_ServiceSubCategory(Convert.ToInt32(hdn_rpt_id.Value));
            DataTable dt = ds.Tables[0];
            lbl_code.Text = dt.Rows[0]["Code"].ToString();
            txt_name.Text = dt.Rows[0]["Name"].ToString();
            txtArabicName.Text = dt.Rows[0]["ArabicName"].ToString();
            drpServiceCategory.SelectedValue = dt.Rows[0]["ServiceCategoryId"].ToString();
        ...
        {
            int res = obj_master.Insert_Update_ServiceSubCategory(Convert.ToInt32(hdn_id.Value), 
                txt_name.Text,Convert.ToInt32(drpServiceCategory.SelectedValue), txt_desc.Text,
        ...
        {
            int res = obj_master.Delete_ServiceSubCategory(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
            ...
            {
                lblerrormsg.Text = "Unable to delete. Entry may be used in service master !..";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDivError();", true);
            }
            pnl_add.Visible = false;
            Upd_Add_Panel.Update();
        ...
        {
            DataTable dt = obj_common.Get_Code(33);
            if (dt.Rows.Count > 0)
        ...
        public void Clear()
        {
            Get_Code();
            txt_name.Text = "";
            drpServiceCategory.ClearSelection();
            drpServiceCategory.Text = "";
            txt_desc.Text = "";
            hdn_id.Value = "0";
            txtArabicName.Text = "";
        ...
        public void previlage_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    int val = obj_common.Form_Previlage_Validation(33, Convert.ToInt32(hdn_user_id.Value));
                    if (val == 0)
                    {
                        Response.Redirect("../Landing.aspx");
                    }
        ...
        public void previlage_action_check()
        {
            try
            {
                if (hdn_user_id.Value != null)
                {
                    ...
                    DataTable dt = obj_common.Action_Previlage_Validation(33, Convert.ToInt32(hdn_user_id.Value));
                    if (dt.Rows.Count > 0)
                    {
