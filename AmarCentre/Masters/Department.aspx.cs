...
namespace AmarCentre.Masters
{
    public partial class Department : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = obj_master.Get_List_Department(page_number, page_size, filter, column, order);
        ...
        {
            DataTable dt = obj_master.Get_List_Department_Excel();
        ...
        protected void rpt_list_OnItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            DataSet ds = obj_master.Edit_Department(Convert.ToInt32(hdn_rpt_id.Value));
        ...
        {
            int res = obj_master.Insert_Update_Department(Convert.ToInt32(hdn_id.Value), txt_name.Text, txt_desc.Text,
        ...
        {
            int res = obj_master.Delete_Department(Convert.ToInt32(hdn_id.Value), Convert.ToInt32(hdn_user_id.Value));
            if (res == 1)
            {
                grid_fill(Convert.ToInt32(lbl_page_number.Text), Convert.ToInt32(drp_count.SelectedValue), hdn_filter.Value, Common_order_column.Value, Common_asc_desc.Value);
                Clear();
            ...
            {
                lblerrormsg.Text = "Unable to delete. Entry may be used in servicemaster !..";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "ToggleDivError();", true);
            }
            pnl_add.Visible = false;
            Upd_Add_Panel.Update();
